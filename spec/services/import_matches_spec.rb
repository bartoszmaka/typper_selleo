...
describe ImportMatches do
  ...
  context 'when one round was completed after call first' do
    it 'will update score' do
      ...
      allow(GoalComWrapper).to receive(:call).with(1) { result_from(matches_table_page_1) }
      allow(GoalComWrapper).to receive(:call).with(2) { result_from(matches_table_page_2) }
      ...
      expect(FootballMatch.all).to include(
        have_attributes(match_data: DateTime.parse("2017/09/10 12:30"), home_team_id: monaco.id, away_team_id: porto.id, home_team_score: nil, away_team_score: nil, round_id:round_first.id ),
        have_attributes(match_data: DateTime.parse("2017/09/11 12:45"), home_team_id: real.id, away_team_id: barcelona.id, home_team_score: nil, away_team_score: nil, round_id: round_first.id),
        have_attributes(match_data: DateTime.parse("2017/11/20 12:30"), home_team_id: monaco.id, away_team_id: real.id, home_team_score: nil, away_team_score: nil, round_id: round_second.id),
        have_attributes(match_data: DateTime.parse("2017/11/21 12:45"), home_team_id: barcelona.id, away_team_id: porto.id, home_team_score: nil, away_team_score: nil, round_id: round_second.id),
      )
      ...
      allow(GoalComWrapper).to receive(:call).with(1) { result_from(matches_table_page_1_after_call) }
      allow(GoalComWrapper).to receive(:call).with(2) { result_from(matches_table_page_2_after_call) }
      ...
      expect(FootballMatch.all).to include(
        have_attributes(match_data: DateTime.parse("2017/09/10 12:30"), home_team_id: monaco.id, away_team_id: porto.id, home_team_score: 1, away_team_score: 5, round_id:round_first.id ),
        have_attributes(match_data: DateTime.parse("2017/09/11 12:45"), home_team_id: real.id, away_team_id: barcelona.id, home_team_score: nil, away_team_score: nil, round_id: round_first.id),
        have_attributes(match_data: DateTime.parse("2017/11/20 12:30"), home_team_id: monaco.id, away_team_id: real.id, home_team_score: nil, away_team_score: nil, round_id: round_second.id),
        have_attributes(match_data: DateTime.parse("2017/11/21 12:45"), home_team_id: barcelona.id, away_team_id: porto.id, home_team_score: nil, away_team_score: nil, round_id: round_second.id),
      )
